...
class ValidRequestManager(RequestManager): pass
class ValidDataManager(DataManager): pass

class ValidRestHandler(RestHandler):
    model = ModelObject
    request_manager = ValidRequestManager
    data_manager = ValidDataManager


class RestHandlerMetaclassTests(unittest.TestCase):
    def test_it_copies_the_model_to_the_data_manager(self):
        self.assertEquals(ModelObject, ValidRestHandler.data_manager.model)
    ...
    def test_it_must_raise_an_error_if_custom_handler_does_not_have_model_attribute(self):
        try:
            class InvalidHandler(ValidRestHandler):
                model = None
                def __init__(self): pass
            InvalidHandler()
            raise Exception('invalid validation')
    ...
    def test_it_create_custom_handler_correctly(self):
        class CustomHandler(ValidRestHandler):
            model = ModelObject
...
class TemplatePathTests(unittest.TestCase):
    ...
    def test_default_template_path_is_the_model_name_as_lower_case(self):
        self.assertEquals('modelobject/', ValidRestHandler.template_path)
